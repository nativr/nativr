...
page, coordinate-window, RGBA-raster, resolved line-segment, and resolved legend records through the
evaluation context; the wire-only protocol transports them; and the Playground maps them onto
Canvas. Runtime and base packages never import DOM or Canvas APIs. `grDevices::as.raster` prepares
row-first character raster values from owned matrices, vectors, and RGB(A) arrays, and the existing
...
bounded, session-owned display list records emitted page/window/raster/segments/legend commands.
