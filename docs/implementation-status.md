...
- Unchanged external-package execution is pinned for `pkgconfig 2.0.3`, `generics 0.1.4`,
  ...
  `numDeriv 2016.8-1.1`, `abind 1.4-8`, and `rprojroot 2.1.1`. Completed tiers and blockers are
  generated from the package corpus rather than collapsed into a supported/unsupported label. The
  latest proofs cover package-owned S3 dispatch, a generated `with_options()` wrapper using
  ...
  composition plus virtual-filesystem root discovery. The next untouched P0 pair is rstudioapi and
  inline.
...
- Vitest currently passes 14 files and 476 tests; one opt-in file with five external-package tests
...
- Checked-in conformance passes 851/851 cases. The optional black-box R oracle passes all 810
