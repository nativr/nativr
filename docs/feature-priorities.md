...
30 available days from 2026-06-29 through 2026-07-28.

This evidence guides NativR's browser-native subset. It does not promise compatibility with the
sampled packages, and arbitrary R package installation remains out of scope.
...
![Highest-reach GNU R core callable names absent from the NativR registry](figures/core-callable-priority.svg)

The coarse feature groups are all runnable, so the collector also counts named-call syntax and
filters names through the checked-in GNU R 4.6.0 black-box callable inventory. The primary ranking
remains download-weighted package reach; raw occurrence counts are a secondary signal.

| Rank | Missing registered callable | Weighted reach | Packages | Observed calls |
| ---: | --------------------------- | -------------: | -------: | -------------: |
|    1 | `library`                   |          37.0% |       35 |            170 |
|    2 | `tempfile`                  |          26.7% |       26 |             69 |
|    3 | `plot`                      |          19.1% |       20 |            179 |
|    4 | `unlink`                    |          18.0% |       17 |             45 |
|    5 | `system.file`               |          17.1% |       17 |             51 |
|    6 | `require`                   |          16.7% |       15 |             49 |
|    7 | `Sys.sleep`                 |          15.6% |       16 |             47 |
|    8 | `return`                    |          12.8% |       10 |             28 |
|    9 | `R.home`                    |          12.7% |       12 |             23 |
|   10 | `writeLines`                |          12.7% |       11 |             33 |

“Missing” here means absent from the generated builtin registry, not necessarily absent syntax: for
example, some language forms are evaluator-native. Conversely, a registered name is not proof of
complete behavior. `library` ranks first but depends on the package-loading architecture. The
high-reach `file.path` text constructor is now implemented; remaining leading candidates divide
across browser-safe host adapters (`tempfile`, `unlink`, and `system.file`), graphics/device
architecture (`plot`), host timing (`Sys.sleep`), and connection-backed text I/O (`writeLines`). The
statistical-modeling entry `lm` (rank 37, 13.9% weighted reach) is now registered with an initial
end-to-end model path; its `coef` accessor (rank 131, 4.9%) is complete in the same slice.
`print`/`cat` and `head`/`str` were selected from preceding snapshots of this same ranking and now
have executable differential coverage. `identical` and the initial `try`/message/warning/handler
condition slice were then completed from the callable ranking, followed by session `options`,
deterministic `interactive()` host-mode behavior, exact decimal `round`, the real/complex
logarithm-exponential family, `with`/`local` data-mask evaluation, and tolerant recursive
`all.equal`, lazy vectorized `ifelse`, and coherent `any`/`all` logical summaries, followed by lazy
vector/list/matrix/data-frame `subset`. `return` is already an evaluator-native language form and
therefore appears missing only because this table measures the builtin registry. Environment removal
via `rm`, pure vector reversal via `rev`, and the coherent cumulative-summary family led by
`cumsum`, delayed function-exit cleanup through `on.exit`, and attribute-preserving `I` class
marking, closure-body inspection, and recursive `unlist` flattening are now complete, followed by
lazy frame `transform` and trailing `tail` selection; file-writing entries remain host-adapter work.
Dynamic caller lookup through `parent.frame` and matrix transpose through `t` are now complete.
Closure `formals` inspection (rank 135, 4.8% weighted reach) and lazy `replicate` evaluation (rank
148, 4.5% weighted reach) are now complete as well. Grouped `split` (rank 155) and real-vector
