...
`is.na`, and `is.nan`. See [`docs/compatibility-contract.md`](docs/compatibility-contract.md) for
exact boundaries.

Source releases are managed with Changesets. npm publication uses GitHub Actions trusted publishing
without a long-lived registry token; see [`docs/releasing.md`](docs/releasing.md).
...
[machine-local dependency setup](docs/development.md#keep-dependencies-outside-a-synchronized-source-folder)
...
evaluation, filesystem access, or network access. Planned directions are documented in
