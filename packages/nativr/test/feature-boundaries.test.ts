...
describe("measured feature boundary coverage", () => {
  it("covers attribute, sequence, and repetition variants", async () => {
    ...
    await expect(runtime.eval("attributes(y ~ x)")).resolves.toEqual(["formula"]);
    ...
    await expect(runtime.eval("names(function(x) x)")).rejects.toMatchObject({ code: "NRT3106" });
    await expect(runtime.eval('setNames(1:2, "a")')).rejects.toMatchObject({ code: "NRT3004" });
    await expect(runtime.eval("dimnames(function(x) x)")).rejects.toMatchObject({
      code: "NRT3153",
    });
  ...
  it("covers random and string option boundaries", async () => {
    ...
    ).resolves.toEqual(["1.200000e+0", NA]);
    await expect(
    ...
    await expect(runtime.eval("rchisq(1, 0)")).rejects.toMatchObject({ code: "NRT3172" });
    await expect(runtime.eval("rt(1, 0)")).rejects.toMatchObject({ code: "NRT3173" });
    await expect(runtime.eval("rexp(1, 0)")).rejects.toMatchObject({ code: "NRT3174" });
  ...
  it("covers matrix, frame, factor, and statistics boundaries", async () => {
    ...
    await expect(runtime.eval("cov(1:3)")).resolves.toBe(1);
    ...
    await expect(runtime.eval("names(data.frame(1:2, y = 3:4))")).resolves.toEqual(["X1", "y"]);
    ...
    await expect(runtime.eval("rbind(list(1))")).rejects.toMatchObject({ code: "NRT3160" });
    ...
    await expect(runtime.eval("data.frame(x = list(1))")).rejects.toMatchObject({
      code: "NRT3127",
    });
  ...
  it("covers apply, date, and sorting option boundaries", async () => {
    ...
    await expect(runtime.eval('strptime("1970-01-01 12:30", "%Y-%m-%d %H:%M")')).resolves.toBe(
      45_000,
    );
  ...
  it("covers indexing, dispatch, and object-system boundaries", async () => {
    ...
    await expect(runtime.eval('R6Class("Bad", public = 1)')).rejects.toMatchObject({
      code: "NRT3165",
    });
    await expect(runtime.eval('new_vctr("x", class = "v")')).resolves.toEqual("x");
    await expect(runtime.eval("new_vctr(function(x) x)")).rejects.toMatchObject({
      code: "NRT3162",
    });
